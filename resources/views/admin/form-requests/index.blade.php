...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">View and manage submitted patient forms</p>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white">
            <form action="{{ route('admin.form-requests.index') }}" method="GET" class="row g-3">
                <div class="col-md-4">
                ...
                @if(request()->hasAny(['search', 'status']))
                    <div class="col-md-2">
                        <a href="{{ route('admin.form-requests.index') }}" class="btn btn-outline-secondary w-100">
                        ...
                        <tbody>
                            @foreach($formRequests as $formRequest)
                                <tr>
                                    ...
                                    <td class="text-end">
                                        <a href="{{ route('admin.form-requests.show', $formRequest) }}"
                                           class="btn btn-sm btn-outline-primary">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        @if(!$formRequest->isCompleted())
                                            <form action="{{ route('admin.form-requests.resend', $formRequest) }}"
                                        ...
                                        @endif
                                        <form action="{{ route('admin.form-requests.destroy', $formRequest) }}"
                                              method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-outline-danger"
                                                    onclick="return confirm('Delete this form request?')">
