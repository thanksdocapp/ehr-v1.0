...
class PatientAlertsController extends Controller
{
    ...
    public function index(Patient $patient): View
    {
        ...
        return view('staff.patients.alerts.index', compact('patient', 'alerts', 'alertCategories', 'severityColors'));
    }
    ...
    public function create(Patient $patient): View
    {
        ...
        return view('staff.patients.alerts.create', compact('patient', 'alertCategories', 'alertTypes', 'alertSeverities'));
    }
    ...
    public function store(Request $request, Patient $patient): RedirectResponse
    {
        ...
        // Validate code exists in category config
        if ($categoryConfig) {
            // Code is valid for this type
        } else {
            // Allow custom codes but warn
        }
        ...
        // Check if user can set restricted flag
        if (isset($validated['restricted']) && $validated['restricted'] && !Auth::user()->is_admin && Auth::user()->role !== 'admin' && Auth::user()->role !== 'doctor') {
            $data['restricted'] = false; // Override if not allowed
        }
        ...
        return redirect()->route('staff.patients.alerts.index', $patient)
    ...
    public function show(Patient $patient, PatientAlert $alert): View
    {
        ...
        return view('staff.patients.alerts.show', compact('patient', 'alert'));
    }
    ...
    public function edit(Patient $patient, PatientAlert $alert): View
    {
        ...
        return view('staff.patients.alerts.edit', compact('patient', 'alert', 'alertCategories', 'alertTypes', 'alertSeverities'));
    }
    ...
    public function update(Request $request, Patient $patient, PatientAlert $alert): RedirectResponse
    {
        ...
        // Check if user can set restricted flag
        if (isset($validated['restricted']) && $validated['restricted'] && !Auth::user()->is_admin && Auth::user()->role !== 'admin' && Auth::user()->role !== 'doctor') {
            unset($validated['restricted']); // Don't allow change
        }
        ...
        return redirect()->route('staff.patients.alerts.show', [$patient, $alert])
