...
class PatientsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            try {
                ...
                \Log::error('Error parsing date of birth', [
                    'date' => $request->date_of_birth,
                    'error' => $e->getMessage()
                ]);
                return redirect()->back()
                    ->withInput()
                    ->withErrors(['date_of_birth' => 'Invalid date format.'])
                    ->with('error', 'Please enter a valid date of birth.');
            }
            
            if ($dateOfBirth) {
                // Double-check date is not in future (shouldn't happen after validation)
                if ($dateOfBirth->isFuture()) {
                    \Log::warning('Future date detected after validation', [
                ...
                $age = $dateOfBirth->age;
                // IMPORTANT: Only require guardian ID for VALID ages between 0 and 17
                $isUnder18 = ($age >= 0 && $age < 18);
                
                \Log::info('Patient age validation', [
                    'date_of_birth' => $dateOfBirth->toDateString(),
                    'calculated_age' => $age,
                    'is_under_18' => $isUnder18,
                    'guardian_id_will_be_required' => $isUnder18,
                    'has_guardian_file' => $request->hasFile('guardian_id_document')
                ]);
            } else {
                \Log::warning('No date of birth provided');
            }
            ...
            \Log::info('Validation rules for guardian ID', [
                ...
                'has_guardian_document' => $request->hasFile('guardian_id_document'),
                'all_files' => array_keys($request->allFiles())
            ]);
            
            // Custom error messages
            $messages = [
                'guardian_id_document.required' => 'Guardian/Parent ID document is required for patients under 18 years old (Age: ' . ($age ?? 'unknown') . ' years).',
            ];
        ...
        if ($user->role === 'doctor') {
            ...
            if ($doctor) {
                ...
                // Only set departments if not already set by the form
                if (empty($departmentIds)) {
                    // Auto-assign doctor's primary department only (not all departments)
                    // This ensures patients are assigned to the specific department selected, not all doctor's departments
                    $primaryDept = $doctor->primaryDepartment();
                    if ($primaryDept) {
                        $departmentIds = [$primaryDept->id];
                        $data['department_id'] = $primaryDept->id;
                    } elseif ($doctor->department_id) {
                        $departmentIds = [$doctor->department_id];
                        $data['department_id'] = $doctor->department_id;
                    }
                } else {
                    // Use provided departments from form, ensure department_id is set
    ...
    public function convertGuest(Request $request, Patient $patient)
    {
        ...
        try {
            ...
            return redirect()->route('staff.patients.show', $patient)
                ->with('success', 'Provisional profile completed — patient record is now full.');
        } catch (Exception $e) {
