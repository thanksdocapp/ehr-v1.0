...
class TwoFactorController extends Controller
{
    ...
    public function showSetup(Request $request)
    {
        ...
        // Check if 2FA is actually required but not enabled (true forced state)
        $isForcedSetup = $isRequired && !$this->twoFactorService->requiresTwoFactor($user);

        // Pass forced setup status to view (and make it available to layout)
        view()->share('isForced2FASetup', $isForcedSetup);

        return view('staff.auth.two-factor-setup', compact('status', 'isRequired', 'isForced', 'isForcedSetup'));
    }
    ...
    public function showVerify(Request $request)
    {
        ...
        return view('staff.auth.two-factor-verify');
    }
    ...
    public function verifyRecovery(Request $request)
    {
        ...
        if ($this->twoFactorService->verifyRecoveryCode($user, $request->recovery_code)) {
            ...
            \App\Models\UserActivity::log([
                'user_id' => $user->id,
                'action' => 'login',
                'description' => 'User logged in successfully with 2FA recovery code',
                'severity' => 'medium',
            ]);
